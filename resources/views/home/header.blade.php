<!--HEADER ENCABEZADO -->
<div class="header_main">
    <!--HEADER MOBILE BEGIN-->
    <div class="mobile_menu">
       <nav class="navbar navbar-expand-lg navbar-light bg-light">
          <div class="logo_mobile"><a href="{{ route('homepage') }}"><img src="images/logo.png"></a></div>
          <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarNav" aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
          <span class="navbar-toggler-icon"></span>
          </button>
          <div class="collapse navbar-collapse" id="navbarNav">
             <ul class="navbar-nav">
                <li class="nav-item">
                <a class="nav-link" href="{{ route('homepage') }}">
                <svg xmlns="http://www.w3.org/2000/svg" fill="white" width="25" height="25" viewBox="0 0 50 50">
                <path d="M 25 1.0507812 C 24.7825 1.0507812 24.565859 1.1197656 24.380859 1.2597656 L 1.3808594 19.210938 C 0.95085938 19.550938 0.8709375 20.179141 1.2109375 20.619141 C 1.5509375 21.049141 2.1791406 21.129062 2.6191406 20.789062 L 4 19.710938 L 4 46 C 4 46.55 4.45 47 5 47 L 19 47 L 19 29 L 31 29 L 31 47 L 45 47 C 45.55 47 46 46.55 46 46 L 46 19.710938 L 47.380859 20.789062 C 47.570859 20.929063 47.78 21 48 21 C 48.3 21 48.589063 20.869141 48.789062 20.619141 C 49.129063 20.179141 49.049141 19.550938 48.619141 19.210938 L 25.619141 1.2597656 C 25.434141 1.1197656 25.2175 1.0507812 25 1.0507812 z M 35 5 L 35 6.0507812 L 41 10.730469 L 41 5 L 35 5 z"></path>
                </svg>
                Home
                </a>
                </li>

                <li class="nav-item">
                <a class="nav-link" href="javascript:SendSMS()">
                <svg xmlns="http://www.w3.org/2000/svg" fill="white" width="25" height="25" viewBox="0 0 24 24" >
                <path d="M20 2H4c-1.1 0-2 .9-2 2v16c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zM4 20V4h16v16H4zm2-6h10v2H6v-2zm0-4h12v2H6v-2zm0-4h12v2H6V6z"></path>
                </svg>
                SMS
                </a>
                </li>

                <li class="nav-item">
                <a class="nav-link" href="javascript:openGmail()">
                <svg xmlns="http://www.w3.org/2000/svg" fill="white" width="25" height="25" viewBox="0 0 24 24">
                <path d="M12 12.713l-11.985-7.713v14h24v-14zM12 11l11.985-7h-23.97l11.985 7z"></path>
                </svg>
                GMAIL
                </a>
                </li>

                <li class="nav-item">
                <a class="nav-link" href="https://www.facebook.com/profile.php?id=61551907872313" target="_blank" rel="noopener noreferrer">
                <svg xmlns="http://www.w3.org/2000/svg"  fill="white" width="25" height="25" viewBox="0 0 24 24">
                <path d="M22.675 0h-21.35c-.733 0-1.325.592-1.325 1.325v21.351c0 .732.592 1.324 1.325 1.324h11.494v-9.294h-3.13v-3.622h3.13v-2.671c0-3.1 1.893-4.786 4.66-4.786 1.325 0 2.462.099 2.795.143v3.24h-1.916c-1.506 0-1.796.717-1.796 1.765v2.309h3.588l-.467 3.622h-3.121v9.293h6.116c.732 0 1.325-.592 1.325-1.324v-21.351c0-.733-.593-1.325-1.325-1.325z"></path>
                </svg>
                FACEBOOK
                </a>
                </li>

                <li class="nav-item">
                <a class="nav-link" href="https://www.thumbtack.com/profile/services/469087462542041101/" target="_blank" rel="noopener noreferrer">
                <svg xmlns="http://www.w3.org/2000/svg"  fill="white" width="25" height="25" viewBox="0 0 24 24">
                <text x="6" y="18" font-size="28" font-family="Arial" font-weight="bold">T</text>
                </svg>
                THUMBTACK
                </a>
                </li>

                <li class="nav-item">
                <a class="nav-link" href="https://www.instagram.com/ajfamily_handyman/" target="_blank" rel="noopener noreferrer">
                <svg xmlns="http://www.w3.org/2000/svg" fill="white" width="25" height="25" viewBox="0 0 24 24">
                <path d="M12 2.163c3.204 0 3.584.012 4.849.07 3.252.15 4.771 1.691 4.921 4.921.058 1.265.069 1.645.069 4.849s-.012 3.584-.07 4.849c-.15 3.225-1.664 4.771-4.921 4.921-1.265.058-1.645.07-4.849.07s-3.584-.012-4.849-.07c-3.257-.15-4.771-1.696-4.921-4.921-.058-1.265-.07-1.645-.07-4.849s.012-3.584.07-4.849c.15-3.23 1.669-4.771 4.921-4.921 1.265-.058 1.645-.07 4.849-.07m0-2.163c-3.259 0-3.667.014-4.947.072-4.093.184-6.333 2.42-6.517 6.517-.058 1.28-.072 1.688-.072 4.947s.014 3.667.072 4.947c.184 4.087 2.424 6.333 6.517 6.517 1.28.058 1.688.072 4.947.072s3.667-.014 4.947-.072c4.087-.184 6.333-2.43 6.517-6.517.058-1.28.072-1.688.072-4.947s-.014-3.667-.072-4.947c-.184-4.097-2.424-6.333-6.517-6.517-1.28-.058-1.688-.072-4.947-.072zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zm0 10.162a4 4 0 110-8 4 4 0 010 8zm6.406-10.845a1.44 1.44 0 100 2.88 1.44 1.44 0 000-2.88z"/>
                </svg>
                INSTAGRAM
                </a>
                </li>


                <li class="nav-item">
                <a class="nav-link" href="https://maps.app.goo.gl/DhV9PGrs4cJQjBNF8" target="_blank" rel="noopener noreferrer">
                <svg xmlns="http://www.w3.org/2000/svg" fill="white" width="25" height="25" viewBox="0 0 24 24">
                <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5S10.62 6.5 12 6.5 14.5 7.62 14.5 9 13.38 11.5 12 11.5z"></path>
                </svg>
                SERVICE AREA
                </a>
                </li>

                <li class="nav-item">
                <a class="nav-link" href="{{ route('request') }}">
                <svg xmlns="http://www.w3.org/2000/svg" fill="white" width="25" height="25" viewBox="0 0 24 24">
                <path d="M10 17l5-5-5-5v10z"></path>
                </svg>
                REQUEST SERVICE
                </a>
                </li>
             </ul>
          </div>
       </nav>
    </div>
    <!--HEADER MOBILE FINISH-->
    <!--HEADER PC BEGIN-->
    <div class="container-fluid">
       <div class="logo"><a href="{{ route('homepage') }}"><img src="images/logoAJ.png"></a></div>
       <div class="menu_main">
          <ul>
          <li class="nav-item">
    <a class="nav-link" href="{{ route('homepage') }}">
        <svg xmlns="http://www.w3.org/2000/svg" fill="white" width="25" height="25" viewBox="0 0 50 50">
            <path d="M 25 1.0507812 C 24.7825 1.0507812 24.565859 1.1197656 24.380859 1.2597656 L 1.3808594 19.210938 C 0.95085938 19.550938 0.8709375 20.179141 1.2109375 20.619141 C 1.5509375 21.049141 2.1791406 21.129062 2.6191406 20.789062 L 4 19.710938 L 4 46 C 4 46.55 4.45 47 5 47 L 19 47 L 19 29 L 31 29 L 31 47 L 45 47 C 45.55 47 46 46.55 46 46 L 46 19.710938 L 47.380859 20.789062 C 47.570859 20.929063 47.78 21 48 21 C 48.3 21 48.589063 20.869141 48.789062 20.619141 C 49.129063 20.179141 49.049141 19.550938 48.619141 19.210938 L 25.619141 1.2597656 C 25.434141 1.1197656 25.2175 1.0507812 25 1.0507812 z M 35 5 L 35 6.0507812 L 41 10.730469 L 41 5 L 35 5 z"></path>
        </svg>
        Home
    </a>
</li>

<li class="nav-item">
    <a class="nav-link" href="javascript:openGmail()">
        <svg xmlns="http://www.w3.org/2000/svg"  fill="white" width="25" height="25" viewBox="0 0 24 24">
            <path d="M12 12.713l-11.985-7.713v14h24v-14zM12 11l11.985-7h-23.97l11.985 7z"></path>
        </svg>
        GMAIL
    </a>
</li>

<li class="nav-item">
    <a class="nav-link" href="https://www.facebook.com/profile.php?id=61551907872313" target="_blank" rel="noopener noreferrer">
        <svg xmlns="http://www.w3.org/2000/svg" fill="white" width="25" height="25" viewBox="0 0 24 24">
            <path d="M22.675 0h-21.35c-.733 0-1.325.592-1.325 1.325v21.351c0 .732.592 1.324 1.325 1.324h11.494v-9.294h-3.13v-3.622h3.13v-2.671c0-3.1 1.893-4.786 4.66-4.786 1.325 0 2.462.099 2.795.143v3.24h-1.916c-1.506 0-1.796.717-1.796 1.765v2.309h3.588l-.467 3.622h-3.121v9.293h6.116c.732 0 1.325-.592 1.325-1.324v-21.351c0-.733-.593-1.325-1.325-1.325z"></path>
        </svg>
        FACEBOOK
    </a>
</li>

<li class="nav-item">
    <a class="nav-link" href="https://www.thumbtack.com/profile/services/469087462542041101/" target="_blank" rel="noopener noreferrer">
        <svg xmlns="http://www.w3.org/2000/svg" fill="white" width="25" height="25" viewBox="0 0 24 24">
            <text x="6" y="18" font-size="28" font-family="Arial" font-weight="bold">T</text>
        </svg>
        THUMBTACK
    </a>
</li>

<li class="nav-item">
    <a class="nav-link" href="https://www.instagram.com/ajfamily_handyman/" target="_blank" rel="noopener noreferrer">
        <svg xmlns="http://www.w3.org/2000/svg" fill="white" width="25" height="25" viewBox="0 0 24 24">
            <path d="M12 2.163c3.204 0 3.584.012 4.849.07 3.252.15 4.771 1.691 4.921 4.921.058 1.265.069 1.645.069 4.849s-.012 3.584-.07 4.849c-.15 3.225-1.664 4.771-4.921 4.921-1.265.058-1.645.07-4.849.07s-3.584-.012-4.849-.07c-3.257-.15-4.771-1.696-4.921-4.921-.058-1.265-.07-1.645-.07-4.849s.012-3.584.07-4.849c.15-3.23 1.669-4.771 4.921-4.921 1.265-.058 1.645-.07 4.849-.07m0-2.163c-3.259 0-3.667.014-4.947.072-4.093.184-6.333 2.42-6.517 6.517-.058 1.28-.072 1.688-.072 4.947s.014 3.667.072 4.947c.184 4.087 2.424 6.333 6.517 6.517 1.28.058 1.688.072 4.947.072s3.667-.014 4.947-.072c4.087-.184 6.333-2.43 6.517-6.517.058-1.28.072-1.688.072-4.947s-.014-3.667-.072-4.947c-.184-4.097-2.424-6.333-6.517-6.517-1.28-.058-1.688-.072-4.947-.072zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zm0 10.162a4 4 0 110-8 4 4 0 010 8zm6.406-10.845a1.44 1.44 0 100 2.88 1.44 1.44 0 000-2.88z"/>
        </svg>
        INSTAGRAM
    </a>
</li>


<li class="nav-item">
    <a class="nav-link" href="https://maps.app.goo.gl/DhV9PGrs4cJQjBNF8" target="_blank" rel="noopener noreferrer">
        <svg xmlns="http://www.w3.org/2000/svg" fill="white" width="25" height="25" viewBox="0 0 24 24">
            <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5S10.62 6.5 12 6.5 14.5 7.62 14.5 9 13.38 11.5 12 11.5z"></path>
        </svg>
        SERVICE AREA
    </a>
</li>

<li class="nav-item">
    <a class="nav-link" href="{{ route('request') }}">
        <svg xmlns="http://www.w3.org/2000/svg" fill="white" width="25" height="25" viewBox="0 0 24 24">
            <path d="M10 17l5-5-5-5v10z"></path>
        </svg>
        REQUEST SERVICE
    </a>
</li>


          </ul>
       </div>
    </div>
 </div>
<!--HEADER PC FINISH-->

 <script>
function SendSMS() {
    var num = "+16159811917";
    var message = "I saw your website ... I am interested in a service";
    var url = "sms:" + num + "?body=" + encodeURIComponent(message);

    window.location.href = url;
}
</script>
<script>
function sendEmail() {
            var email = "destinatario@ejemplo.com";  // Dirección de correo del destinatario
            var subject = "Asunto del correo";  // Asunto del correo
            var body = "Hola, este es el cuerpo del correo.";  // Cuerpo del correo

            var mailtoLink = "mailto:" + email + "?subject=" + encodeURIComponent(subject) + "&body=" + encodeURIComponent(body);

            // Abre el enlace en una nueva pestaña
            window.open(mailtoLink, '_blank');

        }


        function openGmail() {
  // Dirección de correo electrónico del destinatario
  var destinatario = "ajfamilyhandyman@gmail.com";

  // Asunto del correo (opcional)
  var asunto = "Extra Information";

  // Cuerpo del correo (opcional)
  var cuerpo = "Hi I am interested on your handyman services, could you give me some extra information?";

  // Crear el enlace con el esquema mailto
  var enlaceMailto = "mailto:" + destinatario + "?subject=" + encodeURIComponent(asunto) + "&body=" + encodeURIComponent(cuerpo);

  // Abrir el enlace en una nueva ventana o pestaña
  window.open(enlaceMailto);
}
</script>
